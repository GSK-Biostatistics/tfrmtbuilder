...
#'
#' @noRd
test_module <- function(mod_name, args_ui = list(), args_server = list()) {

  id <- "mod_id"

  shinyApp(
    fluidPage(
      do.call(paste0(mod_name, "_ui"), c(id = id, args_ui))
    ),
    function(input,output,session){

      vals <- do.call(paste0(mod_name, "_server"), c(id = id, args_server))
