# Footnote plan - top level module

# returns footnote_plan

footnote_plan_ui <- function(id){

  ns <- NS(id)

  tagList(
    fluidRow(
      h3("Footnote Plan", actionButton(ns("reset"), "Reset", icon = icon("undo"))),
      h4("Marks"),
      shinyWidgets::radioGroupButtons(
        inputId = ns("marks"), label = NULL,
        choices = c("numbers", "letters", "standard", "extended"),
        selected = character(0)
      ),
      h4("Footnote Structures"),
      shinyjs::hidden(
        p(id = ns("none"),
          "None supplied.")
      ),
...
#'
#' @noRd
footnote_plan_server <- function(id, data, tfrmt_app){
  ...
  moduleServer(
    ...
    function(input, output, session){
      ...
      observeEvent(input$reset,{
        req(mode()=="done")
        data_bp(data())
        struct_list(tfrmt_app()$footnote_plan$struct_list)
      })
      ...
      observeEvent(tfrmt_app(),{
        ...
        selected <- tfrmt_app()$footnote_plan$marks %||% "numbers"

        shinyWidgets::updateRadioGroupButtons(session, "marks",
                                              selected = selected)
      })
      observeEvent(data(),{
        data_bp(data())
      ...
      output$tbl <- renderUI({
        ...
        struct_list_txt <- map(struct_list(),
                               ~.x %>% format_footnote_struct() %>% {paste0(., collapse = "<br>")})
      ...
      footnote_plan_out <- reactive({
        req(mode()=="done")

        if (!is.null(struct_list())){
          arg_list <- c(struct_list(), list(marks = input$marks))
          do.call("footnote_plan", arg_list)
