...
- New features:
  - Add ability to specify `tfrmt`, `data`, and/or `mockmode` in the
    [`tfrmtbuilder()`](https://gsk-biostatistics.github.io/tfrmtbuilder/reference/tfrmtbuilder.md)
    function call, as an alternative to manually uploading/specifying
    these within the app.
...
- Bug fixes:
  ...
  - Fix issue where sorting cols selections did not trigger
    activation/glow of save/refresh buttons properly
...
- Bug fixes:
  ...
  - Fix issue where user is unable to select `frmt` buttons in
    `body_plan()` via installed package
